...
namespace MediaFileRecorder
{
	...
	void CMediaFileRecorder::StartWriteFileThread()
	{
		...
		if (m_stRecordInfo.is_record_video)
		{
            ...
            m_WriteVideoThread = (HANDLE)_beginthreadex(NULL,0,&CMediaFileRecorder::VideoWriteFileThreadProc,this,0,NULL);
            if (!m_WriteVideoThread)
            {
                DWORD dwErr = GetLastError();
            }
            else
            {
                SetThreadPriority(m_WriteVideoThread, THREAD_PRIORITY_TIME_CRITICAL);
            }
		}
		...
		{
            m_WriteAudioThread = (HANDLE)_beginthreadex(NULL,0,&CMediaFileRecorder::AuidoWriteFileThreadProc,this,0,NULL);
            if (!m_WriteAudioThread)
            {
                DWORD dwErr = GetLastError();
            }
            else
            {
                SetThreadPriority(m_WriteAudioThread, THREAD_PRIORITY_TIME_CRITICAL);
            }
		}
	}
	...
	void CMediaFileRecorder::StopWriteFileThread()
	{
        ...
        {
            m_bRun = false;
            WaitForSingleObject(m_WriteVideoThread, INFINITE);
            CloseHandle(m_WriteVideoThread);
            m_WriteVideoThread = NULL;

            WaitForSingleObject(m_WriteAudioThread, INFINITE);
            CloseHandle(m_WriteAudioThread);
            m_WriteAudioThread = NULL;
        }
	}

    unsigned int WINAPI CMediaFileRecorder::VideoWriteFileThreadProc(void* Param)
    {
        CMediaFileRecorder *pInst = (CMediaFileRecorder*)Param;
        if (pInst)
        {
            pInst->VideoWriteFileThreadProc();
            return 0;
        }
        return -1;
    }
    ...
    unsigned int WINAPI CMediaFileRecorder::AuidoWriteFileThreadProc(void* Param)
    {
        CMediaFileRecorder *pInst = (CMediaFileRecorder*)Param;
        if (pInst)
        {
            pInst->AuidoWriteFileThreadProc();
            return 0;
        } 
        return -1;
    }
	...
	void CMediaFileRecorder::MixAudio(AVFrame* pDstFrame, const AVFrame* pSrcFrame)
	{
		...
		for (uint32_t j = 0;  j < planes; j++)
		{
			...
			for (int i = 0; i < m_pAudioCodecCtx->frame_size; i++)
			{
				if (dst[i] < 0 && src[i] < 0)
				{
					dst[i] = (dst[i] + src[i]) - dst[i] * src[i] / (-(pow(2.0, 32 - 1) - 1));
				}
				else
				{
					dst[i] = (dst[i] + src[i]) - dst[i] * src[i] / pow(2.0, 32 - 1);
				}
			}
		}
	}
